...
int remainderPower2(int x, int n) {
    ...
    int sign=x>>31<<n;
    int mask_=!remainder+set;
    return(sign&mask_)|remainder;
}
/* 
...
unsigned floatAbsVal(unsigned uf) {
	int ret=uf&0x7FFFFFFF;
  	if(ret>0x7F800000) return uf;
  	return ret;
}
/* 
...
int signMag2TwosComp(int x) {
  int mask=x>>31;
  int sign_x=mask<<31;
  int sign_xx=x^sign_x;

  int ret=(sign_xx^mask)+(mask&1);
  return ret;
  return 2;
}
